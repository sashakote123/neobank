...
const NewsButtons: React.FC<Props> = ({ newsArray, setLeftScroll }) => {
    ...
    const rightHandler = () => {
        if (newsArray?.length && showed === newsArray.length + 1) {
            setRightEnd(true);
            return;
        }
        setLeftScroll((prev) => prev - 400);
    ...
    return (
        <div className="news__buttons">
            {!leftEnd ? (
                <div onClick={leftHandler} className="buttons__button-end">
            ...
            {!rightEnd ? (
                <div onClick={rightHandler} className="buttons__button-end">
