...
describe('News', () => {
  ...
  test('У компонента есть свойство transform', () => {
    const { rerender } = render(<NewsSlider offset={0} newsArray={mockNewsData} />);
    expect(screen.getByTestId('list')).toHaveStyle('transform: translateX(0px)');

    rerender(<NewsSlider offset={400} newsArray={mockNewsData} />);
    expect(screen.getByTestId('list')).toHaveStyle('transform: translateX(400px)');
