...
describe('Currency', () => {
  ...
  test('Компонент отрисован с корректными элементами', () => {
    renderWithProvider();
    expect(screen.getByTestId('title')).toHaveTextContent('Exchange rate in Internet bank');
  ...
  test('Отображает сообщение об ошибке при неудачном запросе', () => {
    ...
    expect(screen.getByText('Failed to fetch actual currency')).toBeInTheDocument();
    expect(screen.queryByText('USD:')).not.toBeInTheDocument();
  });
